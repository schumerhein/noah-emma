...
const navItems = [
  ...
  { href: "/search", label: "Zoeken", icon: "search", locked: true },
...
export function Navigation() {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 max-w-md mx-auto h-[calc(5rem+env(safe-area-inset-bottom))] bg-background/95 backdrop-blur-md border-t border-border z-50 flex items-start justify-around px-2 pt-2 pb-safe-bottom">
      {navItems.map(({ href, label, icon, special, locked }) => {
            ...
            <div className="relative">
              ...
              {locked && (
                <div className="absolute -top-1.5 -right-1.5 bg-white dark:bg-slate-900 rounded-full p-0.5 shadow-sm">
                  <span className="material-icons-round text-[10px] text-primary">lock</span>
                </div>
              )}
            </div>
            <span className={cn(
